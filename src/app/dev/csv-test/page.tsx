...
import { parseCsv, rowToPayload, type CsvRow } from "@/lib/csvImport";
...
export default function CsvTestPage() {
  const results = cases.map(({ name, row }) => {
    try {
      const payload = rowToPayload(row);
      return { name, ok: true, payload } as const;
    } catch (e) {
      return { name, ok: false, error: (e as Error).message } as const;
    }
  });
...
Sorting,Sort these molecules by type.,,,,,,Remember,,,,,,"Polymers|Monomers","Amino acids:Monomers|Nucleotides:Monomers|Proteins:Polymers|DNA:Polymers"`;
  ...
  const [parsed, setParsed] = React.useState<Array<{ idx: number; ok: boolean; payload?: unknown; error?: string }>>([]);
  ...
  const runParse = async () => {
    ...
    try {
      ...
      const out = rows.map((row, i) => {
        try {
          const payload = rowToPayload(row);
          return { idx: i + 2, ok: true, payload } as const; // header + 1-based index
        } catch (e) {
          return { idx: i + 2, ok: false, error: (e as Error).message } as const;
        }
      });
      setParsed(out);
    } finally {
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="grid gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-500">Rows parsed: {parsed.length}</span>
        </div>
        {parsed.length > 0 && (
