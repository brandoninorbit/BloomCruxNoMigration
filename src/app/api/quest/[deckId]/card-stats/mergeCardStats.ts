// Pure aggregation helper extracted from route to avoid Next.js route type pollution
// Response row shape: { cardId, attempts, correct, streak, ease, intervalDays, dueAt, bestCorrectness, avgCorrectness, lastAnsweredAt }
...
export function mergeCardStats(
  ...
  function mapBase(r: { card_id: number; attempts: number; correct: number; streak: number; ease: number; interval_days: number; due_at: string | null }): CardStatRow {
    return {
  ...
  const stats = new Map<number, CardStatRow>();
