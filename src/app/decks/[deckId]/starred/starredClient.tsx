...
import type { DeckCard, DeckCardType, DeckBloomLevel, DeckFillMeta, DeckFillMetaV3, DeckSequencingMeta, DeckShortAnswer, DeckStandardMCQ, DeckTwoTierMCQ, DeckCER, DeckCERMeta, DeckCERPart, DeckSortingMeta, DeckCompareContrastMeta } from "@/types/deck-cards";
...
function toBloom(b: DeckBloomLevel | undefined, type: DeckCardType): DeckBloomLevel { return (b ?? defaultBloomForType(type)) as DeckBloomLevel; }
...
export default function StarredClient({ deckId }: { deckId: number }) {
  ...
  return (
    <main className="study-page container mx-auto px-4 py-6">
      <div className="mb-4">
        <h1 className="font-valid text-2xl font-semibold text-slate-900">Starred Study</h1>
        <p className="font-valid text-sm text-slate-600">All starred cards in this deck · One-off mission</p>
      </div>
      <div className="mb-6">
        <QuestProgress current={idx} total={total} color="#f59e0b" label={`Questions left: ${remaining}`} />
      </div>
      {current ? renderStudy(current, onContinue, (p) => trackLocal(p)) : (
...
function renderStudy(c: DeckCard, onContinue: () => void, trackLocal: (p: { cardId: number; bloom: Bloom; correctness?: number; correct?: boolean; responseMs?: number; confidence?: 0|1|2|3; guessed?: boolean; cardType: string }) => void) {
  ...
  const common = { onContinue } as const;
      ...
      <div className="overflow-x-auto"><div className="inline-block min-w-full align-middle"><div className="rounded-xl border border-slate-200 overflow-hidden">
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50"><tr>
            ...
            <th className="px-4 py-3 text-left text-sm font-semibold text-slate-700">{meta.itemA}</th>
            <th className="px-4 py-3 text-left text-sm font-semibold text-slate-700">{meta.itemB}</th>
            ...
            {rows.map((pt, idx) => (
              <tr key={idx} className="align-top">
                ...
                <td className="px-4 py-3">
                  <textarea className={`w-full rounded-lg border p-2 bg-white text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-300 ${checked ? (effectiveRowCorrect(idx) ? "border-green-500" : "border-red-500") : "border-slate-300"}`} rows={2} placeholder={`How does ${pt.feature} relate to ${meta.itemA}?`} value={ccA[idx] ?? ""} onChange={(e) => { if (!checked) setCcA((prev) => ({ ...prev, [idx]: e.target.value })); }} readOnly={checked} />
                </td>
                <td className="px-4 py-3">
                  <textarea className={`w-full rounded-lg border p-2 bg-white text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-300 ${checked ? (effectiveRowCorrect(idx) ? "border-green-500" : "border-red-500") : "border-slate-300"}`} rows={2} placeholder={`How does ${pt.feature} relate to ${meta.itemB}?`} value={ccB[idx] ?? ""} onChange={(e) => { if (!checked) setCcB((prev) => ({ ...prev, [idx]: e.target.value })); }} readOnly={checked} />
                </td>
